...
This document outlines the steps and configurations required to deploy the Astro website to GitHub Pages using a custom domain (`library.arithmoi.org`).
...
6.  **Deploy to gh-pages:** The `peaceiris/actions-gh-pages@v3` action is used to:
    ...
    *   Create a `CNAME` file in the `gh-pages` branch with the value `library.arithmoi.org`.
    ...
    steps:
      ...
      - name: Deploy 🚀
        ...
        with:
          ...
          cname: library.arithmoi.org # Custom domain
```
...
*   **Branch:**
    *   Branch: `gh-pages`
    *   Folder: `/ (root)`
*   **Custom Domain:** Ensure `library.arithmoi.org` is entered and saved here. GitHub will attempt to verify your DNS settings.

## 3. DNS Configuration for `library.arithmoi.org`

To correctly point your custom subdomain `library.arithmoi.org` to your GitHub Pages site, you need to configure DNS records with your DNS provider (e.g., GoDaddy, Namecheap, Cloudflare).

*   **For the subdomain `library.arithmoi.org`:**
    *   **Type:** `CNAME`
    *   **Host/Name:** `library` (or `library.arithmoi.org` depending on your provider's interface)
    *   **Value/Points to/Target:** `arithmoi.github.io` (replace `arithmoi` with your GitHub username or organization name if different)

*   **Important:**
    *   **Do NOT use `A` records** (pointing to GitHub's IP addresses) for subdomains like `library.arithmoi.org` when using GitHub Pages. This can lead to errors like `InvalidARecordError`.
    *   `A` records are typically used for apex domains (e.g., `arithmoi.org` itself).
...
*   **Custom Domain Not Working / DNS Errors (`InvalidARecordError`):**
    *   Double-check your DNS records with your provider. Ensure you are using a `CNAME` record for `library.arithmoi.org` pointing to `[your-username].github.io`.
    *   Allow time for DNS propagation.
    *   Verify the custom domain is correctly entered in your repository's GitHub Pages settings.

By following these steps, your Astro site should build and deploy automatically to `library.arithmoi.org` via GitHub Pages whenever you push changes to the `main` branch. to GitHub Pages
